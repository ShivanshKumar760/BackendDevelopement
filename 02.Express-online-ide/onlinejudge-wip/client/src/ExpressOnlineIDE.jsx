...
- Create a GET route at '/' that returns { message: 'Hello Express!' }
- Create a GET route at '/health' that returns { status: 'OK' }
- Export the app using export default (don't call listen here)`,
...
export default function ExpressOnlineIDE() {
  ...
  const [githubConfig, setGithubConfig] = useState({
    ...
    repo: localStorage.getItem("github_repo") || "express-judge-solutions",
  });
  ...
  useEffect(() => {
    ...
    const hasToken =
      githubConfig.token && githubConfig.username && githubConfig.repo;
  ...
  const pushToGitHub = async () => {
    ...
    try {
      const fileName = `${selectedProject}/task-${
        currentTask.id
      }-${currentTask.title.toLowerCase().replace(/\s+/g, "-")}.js`;
      const content = btoa(unescape(encodeURIComponent(code))); // Base64 encode
      ...
      const response = await fetch(
        `https://api.github.com/repos/${githubConfig.username}/${githubConfig.repo}/contents/${fileName}`,
        {
          ...
          body: JSON.stringify({
            message: `${allPassed ? "✅" : "🔄"} ${selectedProject} - Task ${
              currentTask.id
            }: ${currentTask.title}`,
            content: content,
            ...(sha && { sha }),
      ...
      if (response.ok) {
        const result = await response.json();
        alert(
          `✅ Code pushed to GitHub successfully!\n\nFile: ${fileName}\nCommit: ${result.commit.sha.substring(
            0,
            7
          )}`
        );
      ...
      alert(
        `❌ Failed to push to GitHub: ${error.message}\n\nPlease check your token and repository settings.`
      );
    } finally {
  ...
  const runTests = async () => {
    ...
    try {
      ...
      // Auto-push to GitHub if tests pass and GitHub is connected
      if (result.allPassed && githubConnected) {
        const autoPush = localStorage.getItem("github_auto_push") === "true";
        if (autoPush) {
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 focus:outline-none focus:border-green-500"
                  placeholder="express-judge-solutions"
                />
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Generate at: Settings → Developer settings → Personal access
                  tokens
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  id="autoPush"
                  checked={localStorage.getItem("github_auto_push") === "true"}
                  onChange={(e) =>
                    localStorage.setItem("github_auto_push", e.target.checked)
                  }
                  className="rounded"
                />
                <label htmlFor="autoPush" className="text-sm">
                  Auto-push when tests pass
                </label>
              </div>
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {githubConnected ? "Connected" : "Setup GitHub"}
            </button>
          </div>
        </div>
        <p className="text-sm text-gray-400 mb-3">
          ...
          <div className="p-6">
            ...
            <div className="bg-gray-800 rounded-lg p-6">
              ...
              <div className="bg-green-900 bg-opacity-30 border border-green-700 rounded p-4">
                ...
                <p className="text-sm text-gray-300">
                  Write your Express.js code using ES6 syntax in the editor and
                  click "Run Tests". Your code will be validated against test
                  criteria including route handlers, middleware, proper exports,
                  and code structure.
                </p>
              </div>
            </div>
              ...
              <div className="flex gap-2">
                ...
                {githubConnected && (
                  <button
                    ...
                    className="px-3 py-2 bg-gray-700 hover:bg-gray-600 disabled:bg-gray-800 rounded flex items-center gap-2 transition-colors text-sm"
          ...
          <div className="h-1/3 border-t border-gray-700 flex flex-col">
            ...
            {allPassed && (
              <div className="bg-green-900 border-t border-green-700 p-4 flex items-center gap-3">
                ...
                {githubConnected && !isPushingToGithub && (
                  <button
                    onClick={pushToGitHub}
                    className="px-3 py-1.5 bg-green-700 hover:bg-green-600 rounded text-sm flex items-center gap-1"
                  >
                    <Github className="w-4 h-4" />
                    Push
                  </button>
